...
function treeDropdown($compile){
    ...
    function getOptions(scope, data, level){
        ...
        angular.forEach(data, function(obj){
            var optionLI = angular.element("<li></li>");//selected.
//          var optionA = angular.element("<p ng-class='{selected:selected.id=="+obj.id+"}' class='level-"+level+"'>"+obj.name+"</p>");
            //注释上句代码，解决初始化下拉选择时个别部门带selected属性（默认选中状态），背景变色问题
            var optionA = angular.element("<p ng-class='selected' id="+obj.id+" class='level-"+level+"'>"+obj.name+"</p>");
            optionA.appendTo(optionLI);
